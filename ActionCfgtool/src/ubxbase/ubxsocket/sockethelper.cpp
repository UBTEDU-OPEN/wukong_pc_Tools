...
namespace HttpSocketHelper {
    ...
    bool connectSocket(int volatile &handle, const std::string &remote_host_name, int remote_port_number, int timeout_millisecs)
    {
        ...
        HttpSocketHelper::setSocketBlockingState(handle, false);

        const int result = ::connect(handle, (struct sockaddr*) &server_temp_address, sizeof(struct sockaddr_in));
        if (result < 0) {
            if (result == SOCKET_ERROR && WSAGetLastError() == WSAEWOULDBLOCK) {
                if (HttpSocketHelper::waitForReadiness(handle, false, timeout_millisecs) != 1) {
                    HttpSocketHelper::setSocketBlockingState(handle, true);
                    return false;
                }
            }
        }
        ...
        HttpSocketHelper::setSocketBlockingState(handle, true);
        HttpSocketHelper::resetSocketOptions(handle);
    ...
    bool resetSocketOptions(int handle)
    {
        ...
        return handle > 0 && setsockopt(handle, SOL_SOCKET, SO_RCVBUF, (const char*)&rcvBufSize, sizeof(rcvBufSize)) == 0\
...
#else
                memcpy(&ret, host->h_addr_list[0], host->h_length);
#endif
            }
        }
        return ret;
