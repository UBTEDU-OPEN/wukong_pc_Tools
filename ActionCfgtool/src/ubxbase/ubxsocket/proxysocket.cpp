...
namespace HttpSocket {
    ...
    {
        char buff[MAX_FILE_BUFFER_SIZE] = { 0 };
        ...
        if (!_proxy_config._username.empty())
        {
            ...
            base64_encode(&b64, buf, auth.c_str(), auth.length(), BASE64_IGNORE_SPACE);
            base64_encode_auth = buf;
            //utils::base64Encode(auth, base64_encode_auth);
//            sprintf_s(buff, MAX_FILE_BUFFER_SIZE, "CONNECT %s:%d HTTP/1.1\r\nHost: %s:%d\r\nAuthorization: Basic %s\r\nProxy-Authorization: Basic %s\r\n\r\n",
//                _host_name.c_str(), _port_number, _host_name.c_str(), _port_number, base64_encode_auth.c_str(), base64_encode_auth.c_str());
            snprintf(buff, MAX_FILE_BUFFER_SIZE, "CONNECT %s:%d HTTP/1.1\r\nHost: %s:%d\r\nAuthorization: Basic %s\r\nProxy-Authorization: Basic %s\r\n\r\n",
                     _host_name.c_str(), _port_number, _host_name.c_str(), _port_number, base64_encode_auth.c_str(), base64_encode_auth.c_str());
        }
        else
        {
...
//                _host_name.c_str(), _port_number, _host_name.c_str(), _port_number);
        ...
        SocketUtils::makeLower(buff, strlen(buff));
    ...
    bool ProxySocket::handShakeWithSock4Proxy()
    {
        ...
        sock4_request.ip_address = HttpSocketHelper::getIntAddress(_host_name.c_str());
        sock4_request.other[0] = '\0';

//        if (sock4_request.ip_address == INADDR_NONE)
        if (sock4_request.ip_address == 0)
            return false;
...
//            strcpy_s(auth_request->name, 255, _proxy_config._username.c_str());
            strncpy(auth_request->name, _proxy_config._username.c_str(), 255);
...
//            strcpy_s((char*)auth_request + auth_length, 255, _proxy_config._password.c_str());
            strncpy((char*)auth_request + auth_length, _proxy_config._password.c_str(), 255);
        ...
        second_sock5_request.dest_address = HttpSocketHelper::getIntAddress(_host_name.c_str());
        second_sock5_request.dest_port = ntohs(_port_number);

//        if (second_sock5_request.dest_address == INADDR_NONE)
        if (second_sock5_request.dest_address == 0)
            return false;
