...
void Updater::checkForUpdates() {
    ...
    URestSignFun::addSignRequest(&request, url(), UPDATER_APPID, "", qApp->applicationName());
...
void Updater::onReply (QNetworkReply* reply) {
    ...
    int cnt = document.array().count();
    if ( cnt == 1 ) {
        /* Get update information */
        QJsonObject obj = document.array().at(0).toObject();
        m_openUrl = obj.value ("packageUrl").toString();
        m_changelog = obj.value ("releaseNote").toString();
        m_downloadUrl = obj.value ("packageUrl").toString();
        m_latestVersion = obj.value ("versionName").toString();
    }
